...
import com.ydd.yanshi.adapter.MarkerPagerAdapter;
...
import com.ydd.yanshi.view.CircleImageView;
                            ...
                            if (AppUtils.isAppInstalled(getActivity(), "com.baidu.BaiduMap")) {
                                try {
                                    Intent intent01 = Intent.getIntent("intent://map/direction?" +
                                            //"origin=latlng:"+"34.264642646862,108.95108518068&" +   //夜魅娇人  此处不传值默认选择当前位置
    ...
    public void addMarker(String nickName, final double lat, final double lng, final String id) {
        AsyncUtils.doAsync(this, t -> {
            ...
            try {
                ...
                Drawable drawable = Glide.with(getActivity())
                        .load(url)
                        .centerCrop()
                        .dontAnimate().skipMemoryCache(true) // 不使用内存缓存
                        .diskCacheStrategy(DiskCacheStrategy.NONE) // 不使用磁盘缓存
                        .into(DisplayUtil.dip2px(getActivity(), 40), DisplayUtil.dip2px(getActivity(), 40))
                        .get();
                ...
                c.uiThread(r -> {
                    picker.addMarker(new MapHelper.LatLng(lat, lng), getRoundedCornerBitmap(bitmap), id+"");
                });
